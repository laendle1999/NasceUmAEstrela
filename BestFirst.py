...
class BestFirst:
	...
	def __algoritmo(self):
		sucessores=[]
		if not self.__visitados:
			p=self.__EstadoInicial
			self.__funcHeurisitca(p)
			self.__vizinhos.append(p)
		while(True):
			m=self.__melhorNo()
			if self.__jogadaValida(m):
				estadoAnterior = BestFirst()
				self.__vizinhos.remove(m)
				...
				for c in sucessores:
					...
					if c not in self.__vizinhos and c not in self.__visitados:
